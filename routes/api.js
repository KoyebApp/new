...
loghandler = {
    ...
    nottext3: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'input parameter text3'
    },
    nottheme: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'input parameter theme'
    },
    notusername: {
        status: false,
        creator: `${creator}`,
    ...
    invalidlink: {
        status: false,
        creator: `${creator}`,
        message: 'maybe your link is invalid.'
    },
    invalidkata: {
        status: false,
        creator: `${creator}`,
        message: 'error, maybe the word is not in api.'
    },
    error: {
        status: false,
        creator: `${creator}`,
...
router.post("/apikey", async (req, res, next) => {
  ...
  if(listkey.includes(key)) {
    res.json({
      message: 'apikey is already registered'
    });
  } else {
    listkey.push(key);
    res.json({
      message: `successfully registered ${key} to apikey database`
    });
  }
});
    ...
    TikTokScraper.getUserProfileInfo(username)
        ...
        .catch(e => {
             res.json({
                 status : false,
                 creator : `${creator}`,
                 message : "error, maybe your username is invalid"
             })
         })
   } else {
res.json(loghandler.invalidKey)
...
router.get('/stalk/npm', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!query) return res.json({ status : false, creator : `${creator}`, message : "input parameter query"})
	...
	var type = req.query.type,
		...
		if (type == 'base64' && encode){
				...
				res.json({
					status:false,
					creator: `${creator}`,
					message: "input encode/decode parameters"
				})
			} else {
				res.json(loghandler.error)
			}
...
router.get('/info/cuaca', async(req, res, next) => {
  ...
  if(!apikey) return res.json(loghandler.notparam)
  if(!kota) return res.json({status: false, code: 406, message: 'enter city parameters'})
  if(listkey.includes(apikey)) {
    Cuaca(kota)
    .then((data) => {
      res.json(data)
...
router.get('/muslim/hadits', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!kitab) return res.json({ status : false, creator : `${creator}`, message : "enter the parameters of the book"})
    if (!nomor) return res.json({ status : false, creator : `${creator}`, message : "input parameter number"})
...
router.get('/muslim/quran', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!surah) return res.json({ status : false, creator : `${creator}`, message : "input parameter surah"})
    if (!ayat) return res.json({ status : false, creator : `${creator}`, message : "input parameter ayat"})
